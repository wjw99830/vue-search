...
function mapToPinyin(hanz) {
    for (const [pinyin, hanzes] of Object.entries(Pinyin)) {
        if (hanzes.includes(hanz)) {
            return pinyin;
        }
    }
}

function useSearch(data, opt = {}) {
...
function search(row, keywords, option = {}, path = '') {
    ...
    for (const [prop, col] of Object.entries(row)) {
        ...
        if (includeProps.length > 0 && includeProps.every((includeProp) => !includeProp.match(new RegExp(`^${propPath}`))) &&
            col !== undefined &&
            col !== null &&
